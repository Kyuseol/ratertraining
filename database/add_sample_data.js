...
// ============================================================================
// 실행: node database/add_sample_data.js
...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('frontend/.env 파일에 REACT_APP_SUPABASE_URL과 REACT_APP_SUPABASE_ANON_KEY를 설정하세요.');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    if (essayError) {
      // 이미 존재하는 에세이일 경우 무시
      console.log(`   ⚠️ 에세이 추가 건너뜀: ${essayError.message}`);
      // 기존 에세이 조회
    ...
    for (let essayIdx = 0; essayIdx < 3; essayIdx++) {
    ...
    for (let essayIdx = 1; essayIdx < 4; essayIdx++) {
      for (const rubric of rubrics) {
        scoresToInsert.push({
